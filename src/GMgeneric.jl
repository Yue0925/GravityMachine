...
f1RL, xf1RL = computeLinearRelax(nbvar, nbctr, A, b, c, typemax(Float64), 1) # opt fct 1
minf1RL, maxf2RL = evaluerSolution(xf1RL, c)

# calcule la valeur optimale relachee de f2 seule et le point (z1,z2) correspondant
f2RL, xf2RL = computeLinearRelax(nbvar, nbctr, A, b, c, typemax(Float64), 2) # opt fct 2
